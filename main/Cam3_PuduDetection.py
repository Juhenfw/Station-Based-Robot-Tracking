...
logging.basicConfig(
    level=logging.DEBUG,  # Menambahkan DEBUG untuk pencatatan yang lebih mendalam
...
logger = logging.getLogger("PuduTracker")

# Flag untuk kontrol aplikasi
running = True
reconnect_delay = 5  # detik
max_queue_size = 100  # Batas ukuran queue
cleanup_interval = 3600  # Bersihkan data lama setiap 1 jam (dalam detik)
max_record_age = 86400  # Simpan data maksimal 24 jam (dalam detik)
sec_stop = 3  # Deteksi berhenti setelah 3 detik

# Queue untuk menampung frame dengan batas ukuran
...
class CheckpointTracker:
    def __init__(self, model_path, checkpoint_areas, db_handler=None, location="CAM1"):
        """Mempersiapkan YOLO model dan tracker"""
        try:
        ...
        self.robot_in_pending_db = {}  # To keep track of robot entries waiting for DB IN


    def is_in_area(self, x, y, area_coords):
        """Memeriksa apakah robot berada dalam area tertentu"""
        x1, y1, x2, y2 = area_coords
        return x1 <= x <= x2 and y1 <= y <= y2

    def process_frame(self, frame, conf_threshold=0.7):
        """Memproses frame dengan deteksi objek"""
        try:
            ...
            if results[0].boxes is not None and results[0].boxes.id is not None:
                ...
                for i, (x, y, w, h) in enumerate(boxes):
                    ...
                    if class_id != 0:  # Skip jika bukan robot Pudu
                        continue
                        ...
                        if self.is_in_area(center_x, center_y, coords):
                            ...
                            if key in self.stop_detection and key not in self.stop_already_triggered:
                                if (datetime.now() - self.stop_detection[key]).total_seconds() > sec_stop:
                                    logger.info(f"Robot {track_id} BERHENTI di {area_name} (Masuk: {self.stop_detection[key].strftime('%Y-%m-%d %H:%M:%S')})")
    ...
    def _draw_events(self, frame):
        ...
        for idx, event in enumerate(self.active_events):
            ...
            if elapsed > 5:  # Tampilkan pesan selama 5 detik
    ...
    while running:
        try:
            try:
                ...
                cv2.imshow("Cam 3 - 10.109.61.12", processed_frame)
...
# Konfigurasi Checkpoint Areas untuk frame 640x480
CHECKPOINT_AREAS = {
    # 'Station 5': (int(287 * (640/1920)), int(450 * (480/1080)), int(512 * (640/1920)), int(738 * (480/1080))),
    'Station 6': (int(1000 * (640/1920)), int(210 * (480/1080)), int(1098 * (640/1920)), int(351 * (480/1080))),
    'Station 4': (int(1105 * (640/1920)), int(323 * (480/1080)), int(1198 * (640/1920)), int(457 * (480/1080))),
    # 'Station 10': (int(0 * (640/1920)), int(315 * (480/1080)), int(281 * (640/1920)), int(724 * (480/1080)))
    'Station 10': (int(0 * (640/1920)), int(315 * (480/1080)), int(512 * (640/1920)), int(738 * (480/1080)))
}
...
def main():
    ...
    try:
        ...
        model_path = 'D:/project/magang2025/Pudu_Monitoring_Station/trainedPuduRobot_yolov8n_ver14.pt'
        ...
        tracker = CheckpointTracker(model_path, CHECKPOINT_AREAS, db_handler=db_pool, location="senban")
        ...
        if use_rtsp:
            ...
            url = "C:/Users/YMPI/Downloads/2025-04-16/20250416101932.ts"
        ...
        if 'tracker' in locals():
            logger.info("\nRIWAYAT KUNJUNGAN ROBOT:")
            for robot_id, checkpoints in tracker.robot_records.items():
                logger.info(f"\nRobot {robot_id}:")
                for area, records in checkpoints.items():
                    for rec in records:
                        exit_time = rec['exit'].strftime("%Y-%m-%d %H:%M:%S") if rec['exit'] else "Masih di area"
